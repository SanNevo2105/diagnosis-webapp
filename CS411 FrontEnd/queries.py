...
def get_top_five_symptoms(user_location, input_date, duration, distance):
    ...
    with symptom_list as (
				SELECT *
				FROM User_Queries
				WHERE 2 * 6371 * ASIN(SQRT((1-COS(ST_Y(""" + user_location + """)-ST_Y(location))+COS(ST_Y(""" + user_location + """))*COS(ST_Y(location))*(1-COS(ST_X(""" + user_location + """)-ST_X(location))))/2)) <= """ + distance + """ 
                -- distance calculated using Haversine formula in km, latitude is y and longitude is x
				AND ABS("""+ input_date +""" - date) <= """+ duration + """) 
...
def get_special_rating(): #untested
    query = """
